...
test('selected scenario drives the headline numbers', () => {
  ...
  // fa-IR از جداکنندهٔ هزارگان ٬ (U+066C) استفاده می‌کند، نه ٫
  assert.match(summary.estimate.totalCost, /۴۲۰٬۵۸۴٬۱۳۰/);
  assert.match(summary.estimate.totalCost, /۷۴۴٬۰۹۵٬۳۱۰/);
...
test('summary carries no hardcoded price constants', () => {
  ...
  // تست از .test-build اجرا می‌شود، پس منبع واقعی از ریشهٔ پروژه خوانده می‌شود.
  const sourcePath = require('node:path').join(__dirname, '..', '..', 'lib', 'project-intake.ts');
...
test('engine failure is reported instead of guessed numbers', async (t) => {
  // موتور واقعی در دسترس است؛ برای سنجش مسیر خطا باید مسیر اسکریپت پل را
  // به مقصدی بی‌وجود ببریم. تست به وضعیت محیط وابسته نباشد.
  const previousRoot = process.env.BANNA_PROJECT_ROOT;
  process.env.BANNA_PROJECT_ROOT = require('node:path').join(__dirname, 'no-such-root');
  t.after(() => {
    if (previousRoot === undefined) delete process.env.BANNA_PROJECT_ROOT;
    else process.env.BANNA_PROJECT_ROOT = previousRoot;
  });
...
test('real engine output flows through the bridge end to end', async () => {
  // یکپارچه: Next → پل → pipeline پایتون. اگر موتور در دسترس نباشد skip می‌شود.
  const { isBridgeConfigured } = require('./python-bridge');
  if (!isBridgeConfigured()) return;

  const summary = await generateProjectSummary({ ...INPUT, surfaceArea: '80' });

  assert.equal(summary.source, 'engine');
  assert.equal(summary.scenarios.length, 4);
  assert.ok(summary.wbs.length > 0, 'WBS خالی از موتور برگشت');
  assert.ok(summary.missingPriceCodes.length === 0, 'آیتم بدون قیمت در دیتاست هست');
  assert.match(summary.estimate.totalCost, /تومان/);
});
